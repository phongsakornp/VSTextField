...
class VSTextField: UITextField {
    ...
    class func makeOnlyDigitsString(string: String) -> String {
        ...
        return string.componentsSeparatedByCharactersInSet(NSCharacterSet.decimalDigitCharacterSet().invertedSet).joinWithSeparator("")
    }
    ...
    func textDidChange() {
        ...
        if let superText = super.text {
            if superText.characters.count > _textWithoutSecureBullets.characters.count {
                currentTextForFormatting = _textWithoutSecureBullets + superText.substringFromIndex(superText.startIndex.advancedBy(_textWithoutSecureBullets.characters.count))
            }
                ...
                currentTextForFormatting = _textWithoutSecureBullets.substringToIndex(_textWithoutSecureBullets.startIndex.advancedBy(superText.characters.count))
            }
